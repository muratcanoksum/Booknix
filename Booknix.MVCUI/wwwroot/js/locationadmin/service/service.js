...
function loadServiceForEdit(serviceId) {
    $.get(`/LocationAdmin/Service/Get/${serviceId}`, function (html) {
...
function showAlert(type, message) {
    const alertDiv = $("#service-alert");
    alertDiv.removeClass("bg-green-100 text-green-800 bg-red-100 text-red-800")
...
$(document).off("submit", "#service-add-form").on("submit", "#service-add-form", function (e) {
    ...
    const url = mode === "edit" ? "/LocationAdmin/Service/Update" : "/LocationAdmin/Service/Add";
...
function loadServices(locationId, msg = null) {
    ...
    $.get(`/LocationAdmin/Services`, function (html) {
        $target.html(html);
        $loader.text("");
...
function deleteService(serviceId) {
    ...
    $.ajax({
        type: "POST",
        url: `/LocationAdmin/Service/Delete/${serviceId}`,
        headers: { "RequestVerificationToken": token },
        success: function (msg) {
            loadServices(locationId, msg);
        },
...
$(document).off("click", ".remove-worker").on("click", ".remove-worker", function () {
    const $workerCard = $(this).closest(".flex");
    const workerId = $(this).data("worker-id");
    const serviceId = $(this).data("service-id");
    const workerName = $workerCard.find(".font-medium").text();
...
function removeWorkerFromService(workerId, serviceId) {
    ...
    $.ajax({
        type: "POST",
        url: `/LocationAdmin/Service/RemoveWorker/${serviceId}/${workerId}`,
        headers: { "RequestVerificationToken": token },
        success: function (msg) {
            loadServices(locationId, msg);
        },
...
function setTimeoutAlert(type, selector, message) {
    ...
    $alert.removeClass("hidden bg-green-100 text-green-800 bg-red-100 text-red-800")
        .addClass(type === "s" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800")
        .text(message)
        .fadeIn();
